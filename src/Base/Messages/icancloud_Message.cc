...
icancloud_Message::icancloud_Message(const char *name, int kind): icancloud_Message_Base(name,kind){
...
icancloud_Message::icancloud_Message(const icancloud_Message& other): icancloud_Message_Base(other.getName()){
...
void icancloud_Message::setTraceArraySize(unsigned int size){
	trace.reserve (size);
}


unsigned int icancloud_Message::getTraceArraySize() const{
	return trace.size();
}
...
void icancloud_Message::setTrace(unsigned int k, const TraceComponent& trace_var){
}
...
string icancloud_Message::getChangingState (){
	return state;
}
...
int icancloud_Message::get_component_to_change_size (){
	return change_states_index.size();
}


int icancloud_Message::get_component_to_change (int componentsPosition){
...
int icancloud_Message::getLastGateId (){
...
int icancloud_Message::getLastModuleId (){
...
int icancloud_Message::getCurrentRequest (){
...
cMessage* icancloud_Message::getParentRequest () const{
	return parentRequest;
}


void icancloud_Message::setParentRequest (cMessage* parent){
	parentRequest = parent;
}


string icancloud_Message::traceToString (){
...
string icancloud_Message::contentsToString (bool printContents){
...
string icancloud_Message::operationToString (unsigned int operation){
...
string icancloud_Message::operationToString (){

	return operationToString(getOperation());
}


string icancloud_Message::getIsResponse_string (){
...
string icancloud_Message::getRemoteOperation_string (){
...
bool icancloud_Message::fitWithParent (){
...
bool icancloud_Message::fitWithParent (cMessage *parentMsg){
...
void icancloud_Message::addNodeTrace (string host, vector<TraceComponent> nodeTrace){
...
string icancloud_Message::getHostName (int k){
	return trace[k].first;
}


vector<TraceComponent> icancloud_Message::getNodeTrace (int k){
	return trace[k].second;
}


void icancloud_Message::parsimPack(cCommBuffer *b){
			...
			for (currentModule=0; currentModule<numModules; currentModule++){
				...
				for(listIterator = trace[currentNode].second[currentModule].reqSequence.begin();
					...
					++listIterator){
						...
						b->pack (*listIterator);
						if (DEBUG_PACKING) printf ("packing request = %lu\n", *listIterator);
				}
			}
		}
}
		...
		for (currentNode=0; currentNode<numNodes; currentNode++){
			...
			for (currentModule=0; currentModule<numModules; currentModule++){
				...
				for (currentRequest=0; currentRequest<numRequests; currentRequest++){
					...
					if (DEBUG_PACKING) printf ("unpacking request = %lu\n", req);
