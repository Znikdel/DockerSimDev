...
AbstractStorageCell::AbstractStorageCell() {
    ...
    from_H_StorageManager = NULL;
    to_H_StorageManager = NULL;
...
void AbstractStorageCell::initialize() {
	...
	// Get module parameters
	    ...
	    // Storage size GB
	        storageSizeGB = par("storageSizeGB").doubleValue();
...
void AbstractStorageCell::sendRemoteRequest (icancloud_App_IO_Message* subRequest, Abstract_Remote_FS* cell){

    Enter_Method_Silent();

    int serverID_position;

 	serverID_position = cell->getServerCellPosition(subRequest);

//	if (serverID_position == -1) {
	    serverID_position = cell->getServerToSend(subRequest);
...
//	}
	...
	// Send the message!
	send(subRequest, to_H_StorageManager);
