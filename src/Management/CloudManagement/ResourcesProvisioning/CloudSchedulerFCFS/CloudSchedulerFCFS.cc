...
vector<AbstractNode*> CloudSchedulerFCFS::selectStorageNodes (AbstractRequest* st_req){
    ...
    // Initialize ..
        ...
        if (streq == NULL) throw cRuntimeError("CloudSchedulerFSFC::selectStorageNodes ->can not cast to storage request\n");
...
vector<AbstractNode*> CloudSchedulerFCFS::remoteShutdown (AbstractRequest* req){
    ...
    // Init ..
     ...
     // Cast
        reqVm = dynamic_cast<RequestVM*>(req);
        if (reqVm == NULL) throw cRuntimeError("CloudSchedulerFCFS::remoteShutdown->Error. Casting the request\n");
